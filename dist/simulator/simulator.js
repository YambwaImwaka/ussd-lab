    ...
    async run(inputs) {
        ...
        try {
            ...
            for (const input of inputs) {
                ...
                // Check max screens
                if (this.app.maxScreens &&
                    session.steps.length >= this.app.maxScreens) {
                    session.ended = true;
                    session.endReason = 'max_screens';
                    session.errors.push(`Session exceeded maximum of ${this.app.maxScreens} screens`);
                    break;
                }
                // Check timeout
                if (this.app.sessionTimeout) {
                    ...
                    if (elapsed > this.app.sessionTimeout) {
                        ...
                        session.errors.push(`Session timeout after ${this.app.sessionTimeout}ms`);
                        break;
                    }
                }
